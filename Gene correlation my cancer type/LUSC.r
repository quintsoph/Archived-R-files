...
##LUSC
LUSC<- subset(CPatients, CPatients[,1]=="LUSC")
test<- intersect(LUSC[,2], rownames(Genes))
LUSCcut<- tcomp[which(row.names(tcomp) %in% LUSC[,2]),]
is.na(LUSCcut)<- sapply(LUSCcut, is.infinite)
ALLL1HSandMUT<- LUSCcut
ALLGENES<- data.frame(LUSCcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(LUSCcut[,c("L1HS")] ~ ALLGENES[,x] + LUSCcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], LUSCcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], LUSCcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], LUSCcut[,c("L1HS")], LUSCcut[,c("Mutations")])
...
LUSCtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
LUSCtogether<- merge(LUSCtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
LUSCtogether<- merge(LUSCtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
LUSCtogether<- merge(LUSCtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
LUSCtogether<- merge(LUSCtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
LUSCtogether<- merge(LUSCtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(LUSCtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
LUSCtogether <- LUSCtogether[order(LUSCtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(LUSCtogether, file="LUSCresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
