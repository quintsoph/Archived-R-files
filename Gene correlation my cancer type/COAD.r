...
##COAD
COAD<- subset(CPatients, CPatients[,1]=="COAD")
test<- intersect(COAD[,2], rownames(Genes))
COADcut<- tcomp[which(row.names(tcomp) %in% COAD[,2]),]
is.na(UCECcut)<- sapply(COADcut, is.infinite)
ALLL1HSandMUT<- COADcut
ALLGENES<- data.frame(COADcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(COADcut[,c("L1HS")] ~ ALLGENES[,x] + COADcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], COADcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], COADcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], COADcut[,c("L1HS")], COADcut[,c("Mutations")])
...
COADtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
COADtogether<- merge(COADtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
COADtogether<- merge(COADtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
COADtogether<- merge(COADtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
COADtogether<- merge(COADtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
COADtogether<- merge(COADtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(COADtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
COADtogether <- COADtogether[order(COADtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(COADtogether, file="COADresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
