...
##ESCA
ESCA<- subset(CPatients, CPatients[,1]=="ESCA")
test<- intersect(ESCA[,2], rownames(Genes))
ESCAcut<- tcomp[which(row.names(tcomp) %in% ESCA[,2]),]
is.na(ESCAcut)<- sapply(ESCAcut, is.infinite)
ALLL1HSandMUT<- ESCAcut
ALLGENES<- data.frame(ESCAcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(ESCAcut[,c("L1HS")] ~ ALLGENES[,x] + ESCAcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], ESCAcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], ESCAcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], ESCAcut[,c("L1HS")], ESCAcut[,c("Mutations")])
...
ESCAtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
ESCAtogether<- merge(ESCAtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
ESCAtogether<- merge(ESCAtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
ESCAtogether<- merge(ESCAtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
ESCAtogether<- merge(ESCAtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
ESCAtogether<- merge(ESCAtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(ESCAtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
ESCAtogether <- ESCAtogether[order(ESCAtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(ESCAtogether, file="ESCAresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
