...
##LUAD
LUAD<- subset(CPatients, CPatients[,1]=="LUAD")
test<- intersect(LUAD[,2], rownames(Genes))
LUADcut<- tcomp[which(row.names(tcomp) %in% LUAD[,2]),]
is.na(LUADcut)<- sapply(LUADcut, is.infinite)
ALLL1HSandMUT<- LUADcut
ALLGENES<- data.frame(LUADcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(LUADcut[,c("L1HS")] ~ ALLGENES[,x] + LUADcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], LUADcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], LUADcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], LUADcut[,c("L1HS")], LUADcut[,c("Mutations")])
...
LUADtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
LUADtogether<- merge(LUADtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
LUADtogether<- merge(LUADtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
LUADtogether<- merge(LUADtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
LUADtogether<- merge(LUADtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
LUADtogether<- merge(LUADtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(LUADtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
LUADtogether <- LUADtogether[order(LUADtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(LUADtogether, file="LUADresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
