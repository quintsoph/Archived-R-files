...
##KIRP
KIRP<- subset(CPatients, CPatients[,1]=="KIRP")
test<- intersect(KIRP[,2], rownames(Genes))
KIRPcut<- tcomp[which(row.names(tcomp) %in% KIRP[,2]),]
is.na(KIRPcut)<- sapply(KIRPcut, is.infinite)
ALLL1HSandMUT<- KIRPcut
ALLGENES<- data.frame(KIRPcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(KIRPcut[,c("L1HS")] ~ ALLGENES[,x] + KIRPcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], KIRPcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], KIRPcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], KIRPcut[,c("L1HS")], KIRPcut[,c("Mutations")])
...
KIRPtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
KIRPtogether<- merge(KIRPtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
KIRPtogether<- merge(KIRPtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
KIRPtogether<- merge(KIRPtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
KIRPtogether<- merge(KIRPtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
KIRPtogether<- merge(KIRPtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(KIRPtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
KIRPtogether <- KIRPtogether[order(KIRPtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(KIRPtogether, file="KIRPresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
