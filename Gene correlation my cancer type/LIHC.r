...
##LIHC
LIHC<- subset(CPatients, CPatients[,1]=="LIHC")
test<- intersect(LIHC[,2], rownames(Genes))
LIHCcut<- tcomp[which(row.names(tcomp) %in% LIHC[,2]),]
is.na(LIHCcut)<- sapply(LIHCcut, is.infinite)
ALLL1HSandMUT<- LIHCcut
ALLGENES<- data.frame(LIHCcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(LIHCcut[,c("L1HS")] ~ ALLGENES[,x] + LIHCcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], LIHCcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], LIHCcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], LIHCcut[,c("L1HS")], LIHCcut[,c("Mutations")])
...
LIHCtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
LIHCtogether<- merge(LIHCtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
LIHCtogether<- merge(LIHCtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
LIHCtogether<- merge(LIHCtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
LIHCtogether<- merge(LIHCtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
LIHCtogether<- merge(LIHCtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(LIHCtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
LIHCtogether <- LIHCtogether[order(LIHCtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(LIHCtogether, file="LIHCresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
