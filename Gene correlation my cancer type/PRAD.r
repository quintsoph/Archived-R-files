...
##PRAD
PRAD<- subset(CPatients, CPatients[,1]=="PRAD")
test<- intersect(PRAD[,2], rownames(Genes))
PRADcut<- tcomp[which(row.names(tcomp) %in% PRAD[,2]),]
is.na(PRADcut)<- sapply(PRADcut, is.infinite)
ALLL1HSandMUT<- PRADcut
ALLGENES<- data.frame(PRADcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(PRADcut[,c("L1HS")] ~ ALLGENES[,x] + PRADcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], PRADcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], PRADcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], PRADcut[,c("L1HS")], PRADcut[,c("Mutations")])
...
PRADtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
PRADtogether<- merge(PRADtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
PRADtogether<- merge(PRADtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
PRADtogether<- merge(PRADtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
PRADtogether<- merge(PRADtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
PRADtogether<- merge(PRADtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(PRADtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
PRADtogether <- PRADtogether[order(PRADtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(PRADtogether, file="PRADresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
