...
test<- intersect(BLCA[,2], rownames(Genes))
BLCAcut<- tcomp[which(row.names(tcomp) %in% BLCA[,2]),]
is.na(BLCAcut)<- sapply(BLCAcut, is.infinite)
...
{
	print(x)
	value <-lm(BLCAcut[,c("L1HS")] ~ ALLGENES[,x] + BLCAcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], BLCAcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], BLCAcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], BLCAcut[,c("L1HS")], BLCAcut[,c("Mutations")])
...
BLCAtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
BLCAtogether<- merge(BLCAtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
BLCAtogether<- merge(BLCAtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
BLCAtogether<- merge(BLCAtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
BLCAtogether<- merge(BLCAtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
BLCAtogether<- merge(BLCAtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(BLCAtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
BLCAtogether <- BLCAtogether[order(BLCAtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(BLCAtogether, file="BLCAresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
