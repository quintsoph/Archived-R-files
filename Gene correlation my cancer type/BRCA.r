...
###BRCA
BRCA<- subset(CPatients, CPatients[,1]=="BRCA")
test<- intersect(BRCA[,2], rownames(Genes))
BRCAcut<- tcomp[which(row.names(tcomp) %in% BRCA[,2]),]
is.na(BRCAcut)<- sapply(BRCAcut, is.infinite)
ALLL1HSandMUT<- BRCAcut
ALLGENES<- data.frame(BRCAcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(BRCAcut[,c("L1HS")] ~ ALLGENES[,x] + BRCAcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], BRCAcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], BRCAcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], BRCAcut[,c("L1HS")], BRCAcut[,c("Mutations")])
...
BRCAtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
BRCAtogether<- merge(BRCAtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
BRCAtogether<- merge(BRCAtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
BRCAtogether<- merge(BRCAtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
BRCAtogether<- merge(BRCAtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
BRCAtogether<- merge(BRCAtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(BRCAtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
BRCAtogether <- BRCAtogether[order(-BRCAtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(BRCAtogether, file="BRCAresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
