...
##CHOL
CHOL<- subset(CPatients, CPatients[,1]=="CHOL")
test<- intersect(CHOL[,2], rownames(Genes))
CHOLcut<- tcomp[which(row.names(tcomp) %in% CHOL[,2]),]
is.na(CHOLcut)<- sapply(CHOLcut, is.infinite)
ALLL1HSandMUT<- CHOLcut
ALLGENES<- data.frame(CHOLcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(CHOLcut[,c("L1HS")] ~ ALLGENES[,x] + CHOLcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], CHOLcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], CHOLcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], CHOLcut[,c("L1HS")], CHOLcut[,c("Mutations")])
...
CHOLtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
CHOLtogether<- merge(CHOLtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
CHOLtogether<- merge(CHOLtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
CHOLtogether<- merge(CHOLtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
CHOLtogether<- merge(CHOLtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
CHOLtogether<- merge(CHOLtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(CHOLtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
CHOLtogether <- CHOLtogether[order(CHOLtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(CHOLtogether, file="CHOLresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
