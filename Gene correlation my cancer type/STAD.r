...
##STAD
STAD<- subset(CPatients, CPatients[,1]=="STAD")
test<- intersect(STAD[,2], rownames(Genes))
STADcut<- tcomp[which(row.names(tcomp) %in% STAD[,2]),]
is.na(STADcut)<- sapply(STADcut, is.infinite)
ALLL1HSandMUT<- STADcut
ALLGENES<- data.frame(STADcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(STADcut[,c("L1HS")] ~ ALLGENES[,x] + STADcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], STADcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], STADcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], STADcut[,c("L1HS")], STADcut[,c("Mutations")])
...
STADtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
STADtogether<- merge(STADtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
STADtogether<- merge(STADtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
STADtogether<- merge(STADtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
STADtogether<- merge(STADtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
STADtogether<- merge(STADtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(STADtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
STADtogether <- STADtogether[order(STADtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(STADtogether, file="STADresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
