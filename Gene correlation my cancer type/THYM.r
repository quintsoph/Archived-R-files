...
##THYM
THCA<- subset(CPatients, CPatients[,1]=="THYM")
test<- intersect(THYM[,2], rownames(Genes))
THYMcut<- tcomp[which(row.names(tcomp) %in% THYM[,2]),]
is.na(THYMcut)<- sapply(THYMcut, is.infinite)
ALLL1HSandMUT<- THYMcut
ALLGENES<- data.frame(THYMcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(THYMcut[,c("L1HS")] ~ ALLGENES[,x] + THYMcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], THYMcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], THYMcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], THYMcut[,c("L1HS")], THYMcut[,c("Mutations")])
...
THYMtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
THYMtogether<- merge(THYMtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
THYMtogether<- merge(THYMtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
THYMtogether<- merge(THYMtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
THYMtogether<- merge(THYMtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
THYMtogether<- merge(THYMtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(THYMtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
THYMtogether <- THYMtogether[order(THYMtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(THYMtogether, file="THYMresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
