...
##PAAD
PAAD<- subset(CPatients, CPatients[,1]=="PAAD")
test<- intersect(PAAD[,2], rownames(Genes))
PAADcut<- tcomp[which(row.names(tcomp) %in% PAAD[,2]),]
is.na(PAADcut)<- sapply(PAADcut, is.infinite)
ALLL1HSandMUT<- PAADcut
ALLGENES<- data.frame(PAADcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(PAADcut[,c("L1HS")] ~ ALLGENES[,x] + PAADcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], PAADcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], PAADcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], PAADcut[,c("L1HS")], PAADcut[,c("Mutations")])
...
PAADtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
PAADtogether<- merge(PAADtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
PAADtogether<- merge(PAADtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
PAADtogether<- merge(PAADtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
PAADtogether<- merge(PAADtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
PAADtogether<- merge(PAADtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(PAADtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
PAADtogether <- PAADtogether[order(PAADtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(PAADtogether, file="PAADresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
