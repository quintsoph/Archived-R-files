...
##PCPG
PCPG<- subset(CPatients, CPatients[,1]=="PCPG")
test<- intersect(PCPG[,2], rownames(Genes))
PCPGcut<- tcomp[which(row.names(tcomp) %in% PCPG[,2]),]
is.na(PCPGcut)<- sapply(PCPGcut, is.infinite)
ALLL1HSandMUT<- PCPGcut
ALLGENES<- data.frame(PCPGcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(PCPGcut[,c("L1HS")] ~ ALLGENES[,x] + PCPGcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], PCPGcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], PCPGcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], PCPGcut[,c("L1HS")], PCPGcut[,c("Mutations")])
...
PCPGtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
PCPGtogether<- merge(PCPGtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
PCPGtogether<- merge(PCPGtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
PCPGtogether<- merge(PCPGtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
PCPGtogether<- merge(PCPGtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
PCPGtogether<- merge(PCPGtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(PCPGtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
PCPGtogether <- PCPGtogether[order(PCPGtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(PCPGtogether, file="PCPGresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
