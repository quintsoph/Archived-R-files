...
##HNSC
HNSC<- subset(CPatients, CPatients[,1]=="HNSC")
test<- intersect(HNSC[,2], rownames(Genes))
HNSCcut<- tcomp[which(row.names(tcomp) %in% HNSC[,2]),]
is.na(HNSCcut)<- sapply(HNSCcut, is.infinite)
ALLL1HSandMUT<- HNSCcut
ALLGENES<- data.frame(HNSCcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(HNSCcut[,c("L1HS")] ~ ALLGENES[,x] + HNSCcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], HNSCcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], HNSCcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], HNSCcut[,c("L1HS")], HNSCcut[,c("Mutations")])
...
HNSCtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
HNSCtogether<- merge(HNSCtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
HNSCtogether<- merge(HNSCtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
HNSCtogether<- merge(HNSCtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
HNSCtogether<- merge(HNSCtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
HNSCtogether<- merge(HNSCtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(HNSCtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
HNSCtogether <- HNSCtogether[order(HNSCtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(HNSCtogether, file="HNSCresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
