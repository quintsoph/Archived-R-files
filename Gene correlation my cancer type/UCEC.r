...
##UCEC
UCEC<- subset(CPatients, CPatients[,1]=="UCEC")
test<- intersect(UCEC[,2], rownames(Genes))
UCECcut<- tcomp[which(row.names(tcomp) %in% UCEC[,2]),]
is.na(UCECcut)<- sapply(UCECcut, is.infinite)
ALLL1HSandMUT<- UCECcut
ALLGENES<- data.frame(UCECcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(UCECcut[,c("L1HS")] ~ ALLGENES[,x] + UCECcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], UCECcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], UCECcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], UCECcut[,c("L1HS")], UCECcut[,c("Mutations")])
...
UCECtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
UCECtogether<- merge(UCECtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
UCECtogether<- merge(UCECtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
UCECtogether<- merge(UCECtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
UCECtogether<- merge(UCECtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
UCECtogether<- merge(UCECtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(UCECtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
UCECtogether <- UCECtogether[order(UCECtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(UCECtogether, file="UCECresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
