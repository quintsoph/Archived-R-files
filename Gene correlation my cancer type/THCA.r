...
##THCA
THCA<- subset(CPatients, CPatients[,1]=="THCA")
test<- intersect(THCA[,2], rownames(Genes))
THCAcut<- tcomp[which(row.names(tcomp) %in% THCA[,2]),]
is.na(THCAcut)<- sapply(THCAcut, is.infinite)
ALLL1HSandMUT<- THCAcut
ALLGENES<- data.frame(THCAcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(THCAcut[,c("L1HS")] ~ ALLGENES[,x] + THCAcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], THCAcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], THCAcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], THCAcut[,c("L1HS")], THCAcut[,c("Mutations")])
...
THCAtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
THCAtogether<- merge(THCAtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
THCAtogether<- merge(THCAtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
THCAtogether<- merge(THCAtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
THCAtogether<- merge(THCAtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
THCAtogether<- merge(THCAtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(THCAtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
THCAtogether <- THCAtogether[order(THCAtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(THCAtogether, file="THCAresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
