...
##CESC
CESC<- subset(CPatients, CPatients[,1]=="CESC")
test<- intersect(CESC[,2], rownames(Genes))
CESCcut<- tcomp[which(row.names(tcomp) %in% CESC[,2]),]
is.na(CESCcut)<- sapply(CESCcut, is.infinite)
ALLL1HSandMUT<- CESCcut
ALLGENES<- data.frame(CESCcut[,-c(1,2)])

#pvalues
testpvalues<- function(x)
{
	print(x)
	value <-lm(CESCcut[,c("L1HS")] ~ ALLGENES[,x] + CESCcut[,c("Mutations")], sep="")
	anova(value)$Pr[1]
}
...
for (i in colnames(ALLGENES))
{
	r = NULL
		...
		r <-cor(ALLGENES[,i], CESCcut[,c(2)], method="pearson")
...
for (i in colnames(ALLGENES))
{
	r = NULL
		#cor(x,y)
		r <-cor(ALLGENES[,i], CESCcut[,c(2)], method="spearman")
...
partialpear<- function(x)
{
	...
	m<- NULL
	k<- NULL
	m<- cbind( ALLGENES[,x], CESCcut[,c("L1HS")], CESCcut[,c("Mutations")])
...
CESCtogether<- merge(tpvalues, tpear, by=0, all=TRUE)
CESCtogether<- merge(CESCtogether, tspear, by.y=0, by.x="Row.names", all=TRUE)
CESCtogether<- merge(CESCtogether, tpartialpearresults, by.y=0, by.x="Row.names", all=TRUE)
CESCtogether<- merge(CESCtogether, tpartialspearresults, by.y=0, by.x="Row.names", all=TRUE)
CESCtogether<- merge(CESCtogether, tcoefgene, by.y=0, by.x="Row.names", all=TRUE)
CESCtogether<- merge(CESCtogether, tcoefmut, by.y=0, by.x="Row.names", all=TRUE)

colnames(CESCtogether)=c("Rownames", "Pvalues", "cor_pearson", "cor_spearman", "partial_pearson", "partial_spearman", "Summary_Gene_Coeff", "Summary_Mutation_Coeff")
CESCtogether <- CESCtogether[order(CESCtogether[,c("Pvalues")]),, drop=FALSE]
setwd("/home/quints1/logfoldgenes/matchedgenes/CancerResults")
write.table(CESCtogether, file="CESCresults.txt", sep="\t", quote=FALSE, row.names=FALSE)
