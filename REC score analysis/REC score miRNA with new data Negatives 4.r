...
CHOL<- read.table("NEG coef of CHOL.txt", header=TRUE, row.names=1)
setwd("E:/Bioinformatics Lab/Cancer Data/REC Score Analysis 4/miRNA/ESCASTAD")
ESCASTAD<- read.table("NEG coef of ESCASTAD.txt", header=TRUE, row.names=1)
...
microcut<- function(x)
{
	...
	ESCASTAD<- cbind(ESCASTADNA[c(x),], CHOL)
...
{
	w <- microcut(y)
	colnames(w)<- c("THCA", "PRAD", "LUSC", "LUAD", "LIHC", "KIRP", "KIRC", "KICH", "HNSC", "ESCASTAD", "CHOL", "BRCA", "BLCA")
	#rank miRNA 
	rrw <- rrfunction(w)
...
{
	slice<- microcut(i)
	colnames(slice)<- c("THCA", "PRAD", "LUSC", "LUAD", "LIHC", "KIRP", "KIRC", "KICH", "HNSC", "ESCASTAD", "CHOL", "BRCA", "BLCA")
	tslice<- t(slice)
...
setwd("E:/Bioinformatics Lab/Cancer Data/REC Score Analysis 4/miRNA")
write.table(orderedREC, "OrderedREC miRNA neg 4.txt", quote=FALSE, sep="\t")
