...
COADREAD<- read.table("NEG coef of COADREAD.txt", header=TRUE, row.names=1)
...
tresults<- t(results)
setwd("E:/Bioinformatics Lab/Cancer Data/REC Score Analysis 4/gene2L1HS")
orderedREC<- tresults[order(tresults[,c("REC")]),, drop=FALSE]
write.table(orderedREC, "Ordered Gene REC Scores Negatives 4.txt", sep="\t", quote=FALSE)
...
setwd("E:/Bioinformatics Lab/Cancer Data/REC Score Analysis 4/gene2L1HS")
write.table(totalranks, "ranks in each cancer of each gene Neg 4.txt", sep="\t", quote=FALSE)
