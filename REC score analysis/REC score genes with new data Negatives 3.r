...
ESCA<- read.table("NEG coef of ESCA.txt", header=TRUE, row.names=1)
...
READ<- read.table("NEG coef of READ.txt", header=TRUE, row.names=1)
setwd("E:/Bioinformatics Lab/Cancer Data/REC Score Analysis 3/REC Score Stuff/STAD")
STAD<- read.table("NEG coef of STAD.txt", header=TRUE, row.names=1)
...
write.table(orderedREC, "Ordered Gene REC Scores Negatives 3.txt", sep="\t", quote=FALSE)
...
setwd("E:/Bioinformatics Lab/Cancer Data/REC Score Analysis 3/REC Score Stuff")
write.table(totalranks, "ranks in each cancer of each gene FIX 3 NEG.txt", sep="\t", quote=FALSE)
